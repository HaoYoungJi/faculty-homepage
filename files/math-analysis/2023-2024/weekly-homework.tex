...
\]
其中等号成立的条件: $\frac{a_1}{b_1} = \frac{a_2}{b_2} = \cdots = \frac{a_k}{b_k} $ 或者$a_i, b_i, i = 1, 2, \ldots, k$中至少有一方全为0.
...
\[
\left(\sum\limits_{k=1}^n a_k^2 \right) \left( \sum\limits_{k=1}^n b_k^2\right) - \left( \sum\limits_{k=1}^n |a_k b_k|  \right)^2 = \frac{1}{2} \sum\limits_{k=1}^n \sum\limits_{i=1}^n (|a_k||b_i| - |a_i||b_k|)^2 =  \sum\limits_{k=1}^{n-1} \sum\limits_{j=i+1}^n (|a_k||b_i| - |a_i||b_k|)^2;
\]
...
\item 求$\int \frac{dx}{\sin^3 x + 3 \sin x}$.

\item 计算
\[
\int \frac{\sec x \cdot \csc x}{\ln \tan x} dx 
\]

\item 求$\int \frac{\sin 2x}{1 + e^{\sin^2 x}}dx$.

\item 求
\[
(1) \int \ln(1 + \sqrt x)dx \ \ \ (2) \int \frac{x \ln x}{\sqrt{1 + x^2}}dx
\]

\item 求$I = \int \frac{x+2}{(x^2 + 2x + 2)^2}dx$.

\item 求$I = \int \frac{x^4 + 1}{x^5 + x^4 - x^3 - x^2}dx$.

\item 求$I = \int \frac{x^2}{(1 - x^2)^{\frac{3}{2}}}dx$.
...
\item 计算$I = \int_0^{\frac{\pi}{4}} \frac{x}{1 + \cos 2x} dx$;

\item 计算$I = \int_0^2 x \sqrt{2x - x^2}$.

\item 计算$I = \int_{- \frac{\pi}{2}}^{ \frac{\pi}{2}} \sqrt{\cos x - \cos^3 x} dx$.

\item 计算$\int_0^1 \frac{1}{(1 + 2x^2) \sqrt{1 + x^2}} dx$.

\item 设$f(x) = \int_1^{\sqrt x} e^{-t^2} dt$, 计算积分$\int_0^1 \frac{f(x)}{\sqrt x}dx$.

\item 设$f \in R([a, b])$, 令
\[
I_n = \sum_{i=1}^n \ln \left[ 1 + \frac{1}{n}f\left( \frac{i}{n} \right) \right].
\]
求$\lim_{n \to \infty} I_n$.

\item 设$f$在$[a, b]$上可积. 证明等式
\[
\int_a^b f^2 (x) dx =0
\]
当且仅当对属于$[a, b]$内$f$的一切连续点$x$有$f(x) = 0$成立.
...
\item 设$f(x)$在$[0, 1]$上连续, $f(x) > 0$. 证明:

(1) 存在唯一的$a \in (0, 1)$, 使得$\int_0^a f(t) dt = \int_a^1 \frac{1}{f(t)} dt$.

(2)对任意的自然数$n$, 存在唯一的$x_n \in (0, 1)$, 使得
\[
\int_{\frac{1}{n}}^{x_n} f(t) dt = \int_{x_n}^1 \frac{1}{f(t)} dt \mbox{ 且 \ } \lim_{n \to \infty} x_n = a.
\]

\item 设$f \in C[0, +\infty), a > 0$, 证明:
\[
...
\item 设$f$有连续的导数, 且
\[
0 < f'(x) < \frac{1}{x^2} (1 \leq x < +\infty).
\]
求证$\lim_{n \to \infty}f(n)$存在.

\item 设$a >0$, $f'(x)$在$[0, a]$上连续. 证明:
\[
|f(0)| \leq \frac{1}{a} \int_0^a |f(x)| dx + \int_0^a |f'(x) |dx.
\]

\item 设$h(x)$是$[a, b]$上正值连续函数, 证明:
\[
\int_a^b h(t) dt \cdot \int_a^b \frac{1}{h(t)} dt \geq (b - a)^2.
\]

Tip: 作变形:
\[
(b - a)^2 = \left( \int_a^b \sqrt{h(t)} \cdot \frac{1}{\sqrt{h(t)}} dt \right)^2.
\]

\item 设$f$在$[a, b]$上可积, $g$在$[a, b]$上单调. 如果对任意的$A \in [a, b]$, 有
\[
\Big| \int_a^A f(x) dx \Big| \leq M,
\]
那么
\[
\Big| \int_a^b f(x)g(x) dx \Big| \leq M(|g(a)| + 2 |g(b)|).
\]

\item 设$f(x)$为连续函数, $F(x) = \int_0^{x^2} t f(x^2 -t) dt$, 求$F'(x)$.

\item 求导数$\frac{d}{dx} \left( \int_0^x \arctan t dt \right)^2$.
...
\item 计算反常积分
\[
I = \int_0^{+\infty} \frac{x \ln x}{(1 + x^2)^2} dx. ( = 0)
\]

\item 求由方程$x^2 + xy + y^2 =1$所确定图形的面积.

\item 设$f(x)$在$[0, 1]$上连续, 在$(0, 1)$二阶可导. 且
\[
f(0) \cdot f(1) >0, f''(x) >0 \ (\forall x \in (0, 1)), \int_0^1 f(x) dx =0.
\]
证明: $f$在$(0, 1)$内恰有两个零点.
...
\item (10.1 8) 求两椭圆$\frac{x^2}{a^2} + \frac{y^2}{b^2} =1$和$\frac{x^2}{b^2} + \frac{y^2}{a^2} =1 \ (a >0, b >0)$所围成公共部分的面积.


\begin{proof}

只需计算$y$轴与$y=x$所夹面积, 再乘8即可. 求出两椭圆曲线交点坐标
\[
\left( \frac{ab}{\sqrt{a^2 + b^2}},  \frac{ab}{\sqrt{a^2 + b^2}} \right).
\]
因此
\[
A = 8 \int_0^{ \frac{ab}{\sqrt{a^2 + b^2}}} \left[  b \sqrt{1 - \frac{x^2}{a^2}} - x \right] dx = 8ab \int_0^{\arcsin  \frac{ab}{\sqrt{a^2 + b^2}}} \cos^2 t dt - \frac{4a^2b^2}{a^2 + b^2}
\]
\[
= 4ab \arcsin  \frac{b}{\sqrt{a^2 + b^2}} + 4ab \cdot  \frac{ab}{a^2 + b^2} - \frac{4a^2b^2}{a^2 + b^2} = 4ab \arcsin  \frac{b}{\sqrt{a^2 + b^2}}.
\]

\end{proof}


\item (10.2, 2(3)) 求曲线绕旋转轴所围成立体的体积:
\[
r = a (1 + \cos \theta) \ (a >0)
\]
绕极轴选择.
...
此为心形线. 极轴即$\theta = 0$的直线, 即$x$轴. 其在极轴之上部分的参数方程为:
\[
\begin{cases}
...
于是
\[
V = \pi \int_{- \frac{a}{4}}^{2a} y^2 dx - \pi \int_{- \frac{a}{4}}^0 y^2 dx = \pi a^3 \int_0^{\pi} (\sin^3 \theta + 2 \sin^3 \theta \cos \theta + \sin^3 \theta \cos^2 \theta) (1 + 2 \cos \theta) d \theta = \frac{8}{3} \pi a^3.
\]
...
\item (10.2, 6) 求$0 \leq y \leq \sin x, 0 \leq x \leq \pi$所示平面图形绕$y$轴选择所得立体的体积.
...
利用微元法计算:
\[
V = 2 \pi \int_0^{\pi} x \sin x dx = - 2 \pi (x \cos x - \sin x) \Big|_0^{\pi} = 2 \pi^2.
\]

\end{proof}


\item (10.3, 3) 求$a, b$的值, 使椭圆$x = a \cos t, y = b \sin t$的周长等于正弦曲线$y = \sin x$在$0 \leq x \leq 2\pi$上一段的长.


\begin{proof}

不妨设$a > b$, 令$c = \sqrt{a^2 - b^2}$表示椭圆的半焦距. 则椭圆周长为
\[
s = \int_0^{2 \pi} \sqrt{(-a\sin t)^2 + (b\cos t)^2} dt = \int_0^{2 \pi} \sqrt{a^2 - c^2 \cos^2 t}dt.
\]
正弦曲线的弧长为:
\[
s' = \int_0^{2 \pi} \sqrt{1 + \cos^2 t} dt = \int_0^{2 \pi} \sqrt{2 -\sin^2 t}dt = \int_0^{2 \pi} \sqrt{2 -\cos^2 t}dt.
\]
解得$a = \sqrt 2, b =1$.
...
\item (10.4, 1(3)) 求曲线$\frac{x^2}{a^2} + \frac{y^2}{b^2} =1$绕$y$轴旋转所得旋转曲面的面积.


\begin{proof}

当$x \geq 0$时, 求出$x = \varphi(y) = a \sqrt{1 - \frac{y^2}{b^2}}$. 因此
\[
S = 2 \pi \int_{-b}^b \varphi(y) \sqrt{1 + \varphi'^2(y)} dy = 2 \pi \int_{-b}^b a \sqrt{1 - \frac{y^2}{b^2}} \cdot \sqrt{1 + \frac{a^2 y^2}{b^2 (b^2 - y^2)}} dy
\]
\[
= 2 \pi \int_{-b}^b \frac{a}{b^2} \sqrt{b^4 - (b^2 - a^2)y^2} dy.
\]

当$a = b$时, $S = 4 \pi a^2$;

当$a < b$时, $S = 2 \pi a \left( a + \frac{b^2}{\sqrt{b^2 - a^2}} \arcsin \frac{\sqrt{b^2 - a^2}}{b} \right)$;

当$a > b$时, $S = 2 \pi a \left( a + \frac{b^2}{\sqrt{a^2 - b^2}} \ln \frac{\sqrt{a^2 - b^2} +a}{b} \right)$.
...
\begin{proof}
\[
\int \frac{dx}{\sin^3 x + 3 \sin x} = \int \frac{dx}{\sin x (\sin^2 x + 3)} = \int \frac{\sin x dx}{\sin^2 x (4 - \cos^2 x)} = -\int \frac{dt}{ (1 - t^2)( 4- t^2) } 
\]
\[
= \frac{1}{3} \int \left( \frac{1}{t^2 -1} - \frac{1}{t^2 -4} \right) dx = \frac{1}{6} \ln \Big| \frac{1 - \cos x }{1 + \cos x} \Big| - \frac{1}{12} \ln \frac{2 - \cos x}{2 + \cos x} + C.
\]
\end{proof}
...
\begin{proof}
\[
\int \frac{\sec x \cdot \csc x}{\ln \tan x} dx = \int \frac{\sec^2 x \cdot \csc x}{\sec x \cdot \ln \tan x} dx = \int \frac{d \tan x}{\tan x \cdot \ln \tan x} = \ln |\ln \tan x| + C.
\]
\end{proof}
...
\begin{proof}
注意$d (\sin^2 x) = \sin 2x$, 令$\sin^2 x =t$, 则
\[
\int \frac{\sin 2x}{1 + e^{\sin^2 x}}dx = \int \frac{dt}{1 + e^t} = - \int \frac{d e^{-t}}{e^{-t} + 1} = - \int \frac{d (e^{-t} + 1)}{e^{-t} + 1}
\]
\[
= - \ln (e^{-t} + 1) + C = - \ln (e^{-\sin^2 x} + 1) + C.
\]
\end{proof}
...
(1) 
\[
\int \ln(1 + \sqrt x)dx = x \ln(1 + \sqrt x) - \frac{1}{2} \int \frac{\sqrt x}{1 + \sqrt x}dx= x \ln(1 + \sqrt x) - \frac{1}{2} x + \frac{1}{2} \int \frac{dx}{1 + \sqrt x}.
\]
令$x = t^2$, 则
\[
\int \frac{dx}{1 + \sqrt x} = 2 \int \frac{t dt}{1 + t} = 2t - \int \frac{dt}{1 + t} = 2 \sqrt x - 2 \ln(1 + \sqrt x)  +C.
\]
从而
\[
\int \ln(1 + \sqrt x)dx = (x-1) \ln(1 + \sqrt x) - \frac{x}{2} + \sqrt x +C.
\]

\noindent
(2) 注意$(\sqrt{1 + x^2})' = \frac{x}{\sqrt{1 + x^2}}$, 故
\[
\int \frac{x \ln x}{\sqrt{1 + x^2}}dx = \int \ln x d \sqrt{1 +x^2} = \ln x \sqrt{1 + x^2} - \int \frac{\sqrt{1 + x^2}}{x}dx.
\]
对上式右端第二个积分, 作变换$x = \tan t$, 则有
\[
\int \frac{\sqrt{1 + x^2}}{x}dx = \int \frac{dt}{\sin t \cdot \cos^2 t} = - \int \frac{d \cos t}{\sin^2 t \cdot \cos^2 t} = - \int \frac{d \cos t}{(1 - \cos^2 t) \cos^2 t}
\]
\[
= - \int \frac{du}{u^2(1 - u^2)} = - \int \left( \frac{1}{u^2} + \frac{1}{1-u^2} \right)du = \frac{1}{u} - \frac{1}{2} \ln \Big| \frac{1 + u}{1 - u} \Big| + C 
\]
\[
= \frac{1}{\cos t} - \frac{1}{2} \ln \Big| \frac{1 + \cos t}{1 - \cos t} \Big| + C = \sqrt{1 + x^2} + \ln \frac{\sqrt{1 + x^2} -1}{x} + C.
\]
...
按照步骤即可计算, 作变量代换$t = x+1$, 则
\[
I = \int \frac{x+1 + 1}{[(x+1)^2 +1]^2}dx = \int \frac{t+1}{(t^2+1)^2}dt = \int \frac{tdt}{(t^2 + 1)^2} + \int \frac{dt}{(t^2 + 1)^2}
\]
\[
= -\frac{1}{2} \frac{1}{t^2 + 1} + \frac{1}{2} \left( \frac{t}{t^2 + 1} + \arctan t \right) + C = \frac{t-1}{2(t^2 +1)} + \frac{1}{2} \arctan t +C
\]
\[
= \frac{1}{2} \frac{x}{x^2 + 2x + 2} + \frac{1}{2} \arctan(x+1) + C.
\]
...
注意
\[
x^5 + x^4 - x^3 - x^2 = x^2(x^3 + x^2 - x - 1) = x^2 (x+1) (x^2 -1) = x^2 (x+1)^2 (x-1).
\]
对应的被积函数分解为
\[
\frac{x^4 + 1}{x^5 + x^4 - x^3 - x^2} = \frac{A}{x} + \frac{B}{x^2} + \frac{C}{x-1} + \frac{D}{x+1} + \frac{E}{(x+1)^2}.
\]
\[
\Rightarrow A =1, B = -1, C = \frac{1}{2}, D = - \frac{1}{2}, E = -1.
\]
\[
\Rightarrow I = \ln |x| + \frac{1}{x} + \frac{1}{2} \ln |x-1| - \frac{1}{2} \ln |x+1| + \frac{1}{x+1} +C.
\]
...
将被积函数变形:
\[
\frac{x^2}{(1 - x^2)^{\frac{3}{2}}} = \frac{x^2}{(1+x)^3 \left(\sqrt{\frac{1-x}{1+x}}\right)^3}.
\] 


故令$\sqrt{\frac{1-x}{1+x}} =t$, 则$x + 1 = \frac{2}{t^2 + 1}$, $dx = -\frac{4t}{(t^2 + 1)^2}$. 故
\[
I = -\frac{1}{2} \int \frac{(1 - t^2)^2}{(1 + t^2)t^2} dt = -\frac{1}{2} \int \frac{(1 + t^2)^2 - 4 t^2}{(1 + t^2)t^2}dt = -\frac{1}{2} \left( t - \frac{1}{t} - 4 \arctan t \right) +C
\]
\[
= 2 \arctan \sqrt{\frac{1-x}{1+x}} + \frac{1}{2} \sqrt{\frac{1 + x}{1 - x}} - \frac{1}{2} \sqrt{\frac{1-x}{1+x}} +C.
\]
...
\begin{proof}
\[
I = \int_0^{\frac{\pi}{2}} \left[  \frac{e^x}{1 + e^x} \sin^4 x +  \frac{e^{-x}}{1 + e^{-x}} \sin^4 (-x) \right]dx = \int_0^{\frac{\pi}{2}} \sin^4 x dx = \frac{3}{4} \cdot \frac{1}{2} \cdot \frac{\pi}{2} = \frac{3}{16} \pi.
\]
...
\[
\int_0^{\frac{\pi}{4}} \frac{x}{1 + \cos 2x} dx = \frac{1}{2} \int_0^{\frac{\pi}{4}} x \sec^2 x dx = \frac{1}{2} \left( x \tan x \Big|_0^{\frac{\pi}{4}} - \int_0^{\frac{\pi}{4}} \tan x dx \right) = \frac{\pi}{8} - \frac{\ln 2}{4}.
\]
...
\[
 \int_0^2 x \sqrt{2x - x^2} = \int_{-1}^1 (t+1) \sqrt{1-t^2} dt =  \int_{-1}^1 \sqrt{1-t^2} dt = \frac{\pi}{2}.
\]
...
\[
\int_{- \frac{\pi}{2}}^{ \frac{\pi}{2}} \sqrt{\cos x - \cos^3 x} dx = 2 \int_0^{ \frac{\pi}{2}} \sqrt{\cos x - \cos^3 x} dx = -2 \int_0^{\frac{\pi}{2}} \sqrt{\cos x} d \cos x  = - \frac{4}{3} (\cos x)^{\frac{3}{2}} \Big|_0^{\frac{\pi}{2}} = \frac{4}{3}.
\]
...
令$x = \tan t$, 得
\[
\int_0^1 \frac{1}{(1 + 2x^2) \sqrt{1 + x^2}} dx = \int_0^{\frac{\pi}{4}} \frac{\sec^2 t dt}{(1 + 2 \tan^2 t)\sec t} = \int_0^{\frac{\pi}{4}} \frac{\cos t }{\cos^2 t + \sin^2 t}dt 
\]
\[
= \int_0^{\frac{\pi}{4}} \frac{d \sin t}{1 + \sin^2 t} = \arctan \sin t \Big|_0^{\frac{\pi}{4}} = \arctan \frac{\sqrt 2}{2}.
\]
...
注意此为瑕积分(且显然收敛), 但仍可进行分部积分,

\[
\int_0^1 \frac{f(x)}{\sqrt x}dx = 2 \int_0^1 f(x) d \sqrt x = 2 \sqrt x f(x) \Big|_0^1 - 2 \int_0^1 \sqrt x f'(x) dx
\]
\[
=-2 \int_0^1 \sqrt x e^{-x} \frac{1}{2 \sqrt x} dx = e^{-1} -1.
\]
...
作变形:
\[
I_n = \left( \sum_{i=1}^n \ln \left[ 1 + \frac{1}{n}f \left( \frac{i}{n} \right) \right] -  \sum_{i=1}^n  \frac{1}{n} f \left( \frac{i}{n} \right) \right) + \sum_{i=1}^n  \frac{1}{n} f \left( \frac{i}{n} \right) : = A_n + B_n.
\]
易知$\lim_{n \to \infty} B_n = \int_0^1 f(x) dx$. 设$|f(x) | \leq M$. 利用不等式
\[
| x - \ln (1 + x) | \leq x^2 \ \  (|x| \leq \frac{1}{2}).
\]
得
\[
|A_n| \leq \sum_{i=1}^n f^2 \left( \frac{i}{n} \right) \frac{1}{n^2} \leq M^2 \sum_{i =1}^n \frac{1}{n^2} \to 0 \ (n \to \infty).
\]
即证.
\end{proof}
...
必要性: 设$f$在$x_0$处连续, 但$f(x_0) \neq 0$, 则存在$\delta >0, [x_0 - \delta, x_0 + \delta] \subset [a, b]$, 使得当$|x - x_0| < \delta$时,
\[
|f(x)| > \frac{|f(x_0)|}{2}.
\]
于是, 
\[
\int_a^b f^2(x) dx \geq \int_{x_0 - \delta}^{x_0 + \delta} f^2 (x) dx > \frac{\delta f^2(x_0)}{2} > 0,
\]
得到矛盾.


充分性: 因为$f$可积, 则$f$在$[a, b]$上的全体连续点集$E$在$[a, b]$中稠密. 因此, 根据假设, 对$[a, b]$的任一分割$\pi$, 均可适当地取$\xi_i \in [x_{i-1}, x_i]$使得$f(\xi_i)=0$. 从而积分和
\[
\sum_{i=1}^n f^2(\xi_i) \Delta x_i =0.
\] 
因此
\[
\int_a^b f^2(x) dx = \lim_{|| \pi || \to 0} \sum_{i=1}^n f^2(\xi_i) \Delta x_i =0.
\]
\end{proof}
...
不妨设$f$不恒为0. 则$M : = \max f(x) > 0$, 且$\int_0^1 f(x) dx >0$. 令
\[
F(x) = f(x) - \int_0^x f(t) dt.
\]
则
\[
F(1) = f(1) - \int_0^1 f(t) dt < 0.
\]
设$\eta \in [0, 1)$使得$f(\eta) = M$. 则
\[
F(\eta) = M - \int_0^{\eta} f(t) dt \geq (1 - \eta) M >0.
\]
在$[\eta, 1]$上应用介值定理即得.
\end{proof}
...
(1) 令
\[
F(x) = \int_0^x f(t) dt - \int_x^1 \frac{1}{f(t)} dt.
\]
则
\[
F(0) = - \int_0^1 \frac{1}{f(t)} dt <0, \ F(1) = \int_0^1 f(t) dt >0.
\]
由于$F'(x) = f(x) + \frac{1}{f(x)} >0$, 利用单调性和介值定理即证.


(2) 令
\[
F_n(x) = \int_{\frac{1}{n}}^x f(t) dt - \int_x^1 \frac{1}{f(t)}dt.
\]
在$[\frac{1}{n}, 1]$上使用介值定理, 证得$x_n$的存在性和唯一性. 再证$x_n$收敛到$a$.


注意到对任意自然数$n$,
\[
F_{n+1}(x) - F_n(x) = \int_{\frac{1}{n+1}}^{\frac{1}{n}} f(t) dt >0 \ (\forall x \in (0, 1)),
\]
因此$F_n(x)$对$n$单调增加. 于是再由$F_n(x)$在$[0, 1]$上严格单调递增, 则
\[
F_n(x_n) =0 = F_{n+1}(x_{n+1}) > F_n(x_{n+1}) \Rightarrow x_n > x_{n+1}.
\]
即$\{ x_n \}$单减有下界, 从而可设$\lim_{n \to \infty} x_n = b$. 由于定积分是其上下限变量的连续函数, 令$n \to \infty$, 则
\[
\int_{\frac{1}{n}}^{x_n} f(t) dt = \int_{x_n}^1 \frac{1}{f(t)}dt \Rightarrow \int_0^b f(t) dt = \int_b^1  \frac{1}{f(t)}dt.
\]
利用唯一性$\Rightarrow b = a$.
...
将$a$看作非负变量, 两边就成为变上限积分. 当$a = 0$时两边都等于0, 然后两边对$a$求导即得.
...
由$f'(x)>0$知$f$在$[1, +\infty)$上严格递增, 因
\[
f(x) - f(1) = \int_1^x f'(t) dt \leq \int_1^x \frac{1}{t^2} dt = - \frac{1}{x} +1,
\]
故$f(x) < f(1) + 1 (x >1)$. 根据单调有界原理即证.
\end{proof}
...
利用积分中值定理, $\exists \xi \in [0, a]$, 使得
\[
|f(\xi)| = \frac{1}{a} \int_0^a |f(x)| dx.
\]
又有
\[
f(\xi) - f(0) = \int_0^{\xi} f'(x) dx \Rightarrow |f(0)| \leq |f(\xi)| + \Big| \int_0^{\xi} f'(x) dx \Big|.
\]
联立两式即得.
...
利用积分第二中值的推广形式, 存在$\xi \in [a, b]$, 使得
\[
\int_a^b f(x)g(x) dx = g(a) \int_a^{\xi} f(x) dx + g(b) \int_{\xi}^b f(x) dx = (g(a) - g(b))  \int_a^{\xi} f(x) dx + g(b)  \int_a^b f(x) dx.
\]
取绝对值即得.
\end{proof}
...
注意被积函数中含有变量$x$, 作换元$u = x^2 -t$, 得
\[
F(x) = - \int_{x^2}^0 (x^2 -u) f(u) du = x^2 \int_0^{x^2} f(u)du - \int_0^{x^2} uf(u) du,
\]
求导即得$F'(x) = 2x \int_0^{x^2} f(u) du$.
...
\[
\frac{d}{dx} \left( \int_0^x \arctan t dt \right)^2 = 2 \arctan x  \int_0^x \arctan t dt = 2 \arctan x \left( x \arctan x - \frac{1}{2} \ln (1+x^2) \right).
\]
...
利用分部积分得
\[
\int_a^b f(x) dx = \int_a^b f(x) d (x -a) = f(x) (x - a) \Big|_a^b - \int_a^b (x - a ) f'(x) dx, 
\]
\[
\int_a^b f(x) dx = \int_a^b f(x) d (x -b) = f(x) (x - b) \Big|_a^b - \int_a^b (x - b ) f'(x) dx.
\]

\[
\Rightarrow \int_a^b f(x) dx =  \frac{1}{2} (b - a) (f(a) + f(b)) - \frac{1}{2} \int_a^b (x - a + x - b) f'(x) dx
\]
\[
=  \frac{1}{2} (b - a) (f(a) + f(b)) - \frac{1}{2} \int_a^b f'(x) d (x-a)(x-b)
\]
即证.
...
(1) 从方程中解出
\[
y_{1, 2}(x) = -\frac{x}{2} \pm \sqrt{1 - \frac{3}{4} x^2}, \ -\frac{2}{\sqrt 3} \leq x \leq \frac{2}{\sqrt 3},
\]
再计算定积分
\[
S  = \int_{-\frac{2}{\sqrt 3}} ^{\frac{2}{\sqrt 3}} [y_1 - y_2] dx = \frac{2\pi}{\sqrt 3}.
\]
...
则
\[
S = \frac{1}{2} \int_0^{2 \pi} r^2 d\theta = \int_0^{2\pi} \frac{d \theta}{2 + \sin \theta} = \frac{2\pi}{\sqrt 3}.
\]
...
首先, 易得$f$存在零点$x_1$. 其次, 根据凸函数的性质, $f$在$(0, 1)$内不取最大值, 因此$f(0), f(1)>0$成立. 再根据凸性, $f$有唯一的最小值$\eta \in (0, 1)$. 用介值定理可以得到两个零点$x_1, x_2$. 


假设还有第三个零点, 则使用两次Rolle定理, 得到$f''(x)$的零点, 矛盾.
